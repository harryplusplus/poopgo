...
수백만 명의 AI 작업자가 매번 그 코드를 읽고 "이게 왜 있지?"라고 헷갈린다.
한 사람이 한 번 고민하는 게 아니라, 앞으로 이 파일을 열 모든 에이전트가 매번 낭비한다.
"나중에 필요할지도 모른다"는 유혹에 저항할 것. Git이 기억한다.

### 히스토리는 얇고 단순하게 평탄화한다
코드는 현재 정답만 담는다. fix → revert → re-fix 같은 흔적을 코드에 남기지 않는다.
임시 주석, #ifdef 스타일 우회, "예전 방식" 주석은 Git history에 맡기고 지금 파일에서는 지운다.
이슈 추적과 의사결정 맥락은 커밋 메시지에 명확히 기록한다.
